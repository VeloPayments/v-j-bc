/**
 * \file AgentConnection_getPreviousTransactionIdForTransactionByIdNative.c
 *
 * Get the previous Transaction ID for the given Transaction ID.
 *
 ...
 * Method:    getPreviousTransactionIdForTransactionByIdNative
 * Signature: (Ljava/util/UUID;)Ljava/util/Optional;
 */
JNIEXPORT jobject JNICALL Java_com_velopayments_blockchain_client_AgentConnection_getPreviousTransactionIdForTransactionByIdNative(
    JNIEnv* env, jobject that, jobject txnId)
{
    ...
    /* if the previous transaction id is the zero id, then return empty. */
    transaction_record_t* rec = (transaction_record_t*)val.mv_data;
    if (!memcmp(rec->prev_transaction_uuid, zero_uuid, 16))
    {
        retval = (*env)->CallStaticObjectMethod(env, Optional, Optional_empty);
        goto cleanup_txn;
    }
    ...
    jobject prevTxnId = uuidFromBytes(env, rec->prev_transaction_uuid);
    if (NULL == prevTxnId)
    {
        (*env)->ThrowNew(env, IllegalStateException,
    ...
    /* success. */
    retval =
        (*env)->CallStaticObjectMethod(env, Optional, Optional_of, prevTxnId);
