    ...
    jobject peerPublic, jbyteArray key)
{
    ...
    {
        (*env)->ThrowNew(
            env, native_inst->NullPointerException.classid, "localPrivate");
        return NULL;
    }
    ...
    {
        (*env)->ThrowNew(
            env, native_inst->NullPointerException.classid, "peerPublic");
        return NULL;
    }
    ...
    if (0 != vccrypt_buffer_init(
                    &ivBuffer, &native_inst->alloc_opts, IV_SIZE))
    {
        (*env)->ThrowNew(
            env, native_inst->IllegalStateException.classid,
